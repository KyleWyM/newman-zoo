...
public class MainSinglePlayer {
    ...
    public static void update() {
        ...
        total_income = 0;
        total_maintenance = 0;
        int visitors = ThreadLocalRandom.current().nextInt(reputation * 4, (reputation * 8) + 0);
            if (TicketAlgorithm.ticketprice > visitors){
                total_income = 0;
                IBIO.output(String.format("Your ticket price is too high, lower it or no one will come!"));}
            else if (TicketAlgorithm.ticketprice < visitors) {
                total_income = total_income + visitors * TicketAlgorithm.ticketprice;
                IBIO.output(String.format("You had %s visitors come to your zoo", visitors));
            }
